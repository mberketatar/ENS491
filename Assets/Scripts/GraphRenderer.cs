...
public class GraphRenderer : MonoBehaviour
{
    ...
    public void ShowGraph(List<SpendingEntry> entries)
    {
        ...
        DateTime minDate = entries[0].date;
        DateTime maxDate = entries[entries.Count - 1].date;
        ...
        {
            float xPosition = (float)(entries[i].date - minDate).TotalSeconds / (float)dateRange.TotalSeconds * graphWidth;
            float yPosition = (entries[i].amount - yMinimum) / (yMaximum - yMinimum) * graphHeight;
